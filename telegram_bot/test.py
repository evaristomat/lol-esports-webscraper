...
async def process_bets(mode):
    ...
    if mode == "pending":
        ...
        grouped_bets = pending_bets.groupby(['date', 'league', 't1', 't2'])
        ...
        grouped_changed_bets = changed_bets.groupby(['date', 'league', 't1', 't2'])
...
def get_yesterdays_summary():
    ...
    # Format the summary
    summary = (f"📊 *Yesterday's Summary for {yesterdays_date}:*\n"
               f"✅ *Wins:* {wins} | ❌ *Losses:* {losses}\n"
               f"💰 *Profit:* {profit:.2f} units")
...
async def main():
    try:
        ...
        # Check if messages were sent to prevent sending the summary multiple times
        if MESSAGES_SENT:
